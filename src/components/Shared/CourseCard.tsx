...
export const CourseCard = ({ data }: CardProps) => {
    const status = data.status === 0 ? 'درحال ترجمه' : data.status === 1 ? 'پایان یافته' : 'متوقف شده'
    const statusColor = data.status === 0 ? 'text-primary-400' : data.status === 1 ? 'text-success-400' : 'text-danger-400'
    const link = encodeTitle(data?.urlName);
    const teacherLink = encodeTitle(data?.teacher?.teacherEngName);
    const academyLink = encodeTitle(data?.academy?.academyEngName);



    const originalPrice = data?.price; // قیمت اصلی
    const discount = data?.discount; // آبجکت تخفیف
    ...
    let discountedPrice = hasDiscount && !discountExpired ? originalPrice * (1 - discount.percent / 100) : originalPrice;


    return (
        <div className="w-full max-w-[25rem] h-[27rem] transition-all shadow-medium dark:shadow-xl relative
        hover:shadow-medium
            ...
            <div className="w-full h-full flex flex-col">
                ...
                <div className="w-full h-3/5">
                    <div className="h-full flex flex-col">
                        ...
                        <div className="mt-1 px-3 flex items-center justify-between gap-0.5">
                            <div className="flex items-center">
                                <GoDotFill size={10} className={`${statusColor}`} />
                        ...
                        <div className="px-3 py-1 pb-2 mt-auto flex justify-end">
                            ...
                            {isFree || isFullyDiscounted ? (
                                <div className="flex items-center gap-2">
                                    <span className="text-left font-bold text-secondary-400 text-2xl">رایگان</span>
                                </div>
                            ) :
                                hasDiscount && !discountExpired ? (
                                    <div className="flex items-center gap-3">
                                        <span className="text-left text-gray-700 dark:text-gray-300 font-medium line-through">
    ...
    return (
        ...
        dark:border-0 bg-white dark:bg-[#212123]/70 dark:backdrop-blur-md dark:backdrop-saturate-150 rounded-md cursor-pointer overflow-hidden  ">
            ...
            <div className="w-full h-full flex flex-col">
                ...
                <div className="p-2 overflow-hidden h-[14rem] md:h-[12rem] md:aspect-video">
